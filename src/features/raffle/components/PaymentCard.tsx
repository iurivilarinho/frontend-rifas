...
import type {
  ...
  MercadoPagoOrder,
} from "@/features/reservation/api/dtos/reservation";
...
export const PaymentCard = ({
  ...
  useEffect(() => {
    if (!order?.date_of_expiration) return;
  ...
  const qrBase64 = order?.point_of_interaction?.transaction_data?.qr_code_base64 ?? null;
  ...
  return (
    <Card className="h-full min-h-0 flex flex-col">
      ...
      <CardContent className="flex-1 min-h-0 overflow-y-auto">
        ...
        {!order && (
          <div className="mt-4">
            <Button onClick={handleSubmit} disabled={!userData} className="w-full">
              Gerar Pix
            </Button>
          </div>
        )}
            ...
            <div className="mt-6 flex items-center justify-between rounded-md border p-3">
              ...
              <Dialog>
                <DialogTrigger asChild>
                  <Button
                    disabled={(!qrImgSrc && !ticketUrl) || isExpired}
                    className="shrink-0"
                  >
                    Ver QR Code
                  </Button>
                ...
                <DialogContent>
                  {qrImgSrc ? (
                    <img src={qrImgSrc} alt="QR Code PIX" className="w-full h-auto" />
                  ) : ticketUrl ? (
